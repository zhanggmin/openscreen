...
**所做更改**
- 新增项目归档状态说明和警告信息
- 更新平台差异说明，明确 macOS/Windows 原生录制与 Linux 浏览器录制的区别
- 增强故障排查指南，包含权限升级、沙箱错误等具体解决方案
- 补充项目快速增长背景和开源路线说明
- 更新安装方式概览，包含最新的平台支持信息
...
## 简介
OpenScreen 是一款免费、开源的桌面屏幕录制与视频编辑工具，定位为轻量级替代方案，面向需要快速制作产品演示或教程的用户。该项目起源于个人侧项目，现已获得广泛关注，但目前处于即将归档状态。它基于 Electron 构建，覆盖从录制到导出的完整工作流，并提供跨 macOS、Linux 和 Windows 的原生体验。

**重要更新**：该项目已宣布即将归档，但仍提供完整的功能集，包括屏幕录制、视频编辑和导出等核心功能。
...
- 核心价值主张
  ...
  - 无障碍软件：无付费层级、无功能锁定，完全免费使用。

- 主要功能特性
  - 屏幕/窗口/区域录制；系统音频与麦克风采集。
  - 网页摄像头画中画叠加与自定义形状。
  - 自动或手动缩放区域、可调节深度、持续时间和缓动曲线。
  - 自定义光标大小、平滑度和点击效果，支持光标主题。
  - 自动字幕生成，本地处理无需上传。
...
- [docs/README.md:7-14](file://docs/README.md#L7-L14)
...
### 权限问题解决
- **macOS 权限升级**
  - 如果从旧版本升级遇到权限问题，需要先卸载旧版本，删除系统设置中现有的 OpenScreen 条目，然后重新安装并重新授予权限。
  - 需要"屏幕录制"与"辅助功能"权限；如果 Gatekeeper 阻挡应用，可通过移除隔离属性绕过。
- **Windows 权限**
  - 需要管理员权限与设备权限；SmartScreen 可能显示警告，需要点击"更多信息"然后"仍然运行"。
- **Linux 权限**
  - 需要 PipeWire 或 PulseAudio 支持；某些桌面环境可能需要额外的屏幕录制权限。

### 平台差异问题
- **Linux 浏览器录制限制**
  - Linux 使用浏览器管道录制，不支持自定义光标、原生摄像头捕获和系统音频捕获。
  - 需要在较新的发行版上使用 PipeWire，默认 Ubuntu 22.04+、Fedora 34+ 支持系统音频捕获。

### 沙箱和安全问题
- **AppImage 沙箱错误**
  - 如果 AppImage 启动时出现"沙箱"错误，可以使用 `--no-sandbox` 参数运行。
- **macOS 隔离属性**
  - 通过终端命令移除隔离属性后，需要在系统偏好设置中重新授予权限。

### 导出和录制问题
- **导出失败诊断**
  - 查看导出诊断消息中的原因、源文件名、输出尺寸、帧率、编解码器与比特率；确认浏览器是否支持 VideoEncoder。
- **录制质量优化**
  - macOS 和 Windows 使用原生管道提供更高质量和清洁的窗口级捕获；Linux 通过浏览器管道录制。

**章节来源**
- [README.md:79-81](file://README.md#L79-L81)
- [README.md:147-151](file://README.md#L147-L151)
- [README.md:152-163](file://README.md#L152-L163)

## 结论
OpenScreen 以 Electron 为基础，结合原生辅助进程与 Web 技术，提供了稳定、可扩展的屏幕录制与视频编辑能力。其开源路线与简洁的功能集使其成为 Screen Studio 的理想替代，尤其适合需要控制力与成本效益的用户。

**重要更新**：虽然项目即将归档，但仍保持完整的功能集和活跃的社区支持。随着 macOS/Windows 原生录制的逐步完善，OpenScreen 将进一步提升录制稳定性与时序一致性。
...
**章节来源**
- [README.md:54-146](file://README.md#L54-L146)
...
- [CONTRIBUTING.md:41-45](file://CONTRIBUTING.md#L41-L45)

### 平台差异详细说明
- **录制管道差异**
  - macOS 和 Windows 使用原生管道（ScreenCaptureKit/WGC）提供高质量和清洁的窗口级捕获。
  - Linux 通过浏览器管道录制，功能相对受限。
- **光标支持差异**
  - macOS 和 Windows 捕获真实光标（形状、类型和点击），支持光标主题、点击效果和可编辑光标覆盖层。
  - Linux 仅捕获光标位置（用于自动缩放），不支持光标选项。
- **摄像头支持差异**
  - macOS 和 Windows 原生捕获摄像头；Linux 通过浏览器录制但支持画中画叠加。
- **系统音频支持**
  - macOS 13+ 支持系统音频捕获；macOS 12 及以下仅支持麦克风。
  - Windows 默认支持系统音频。
  - Linux 需要 PipeWire（Ubuntu 22.04+、Fedora 34+）。
