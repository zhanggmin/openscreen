...
- [docs/dev/shouce/demo-builder-enhancement-plan.md](file://docs/dev/shouce/demo-builder-enhancement-plan.md)
</cite>

## 更新摘要
**变更内容**
- 新增DemoService原生服务，扩展原生桥接系统以支持DemoBuilder功能
- 添加DemoBuilder项目管理功能，包括项目创建、加载、保存、删除
- 实现截图导入和管理功能，支持多种图像格式
- 集成DemoBuilder编辑器窗口管理
- 扩展DemoBuilder导出功能（视频、GIF、PDF）
- 更新IPC处理器以支持demo域的所有操作
...
## 简介
本文件系统性阐述 OpenScreen 的原生桥接系统，聚焦 Electron 主进程与渲染器进程之间的原生能力桥接机制。文档覆盖服务层模式、适配器模式的应用，以及统一的跨进程通信策略；深入解析 ProjectService（项目服务）、SystemService（系统服务）、CursorService（游标服务）与 DemoService（演示服务）的职责边界与协作方式；阐明 NativeBridgeStateStore（状态存储）在主进程中的单源真相角色；并提供原生功能扩展与平台适配的最佳实践。
...
DB["DemoBuilder 组件<br/>src/components/demo-builder/*"]
end
subgraph "预加载"
...
- [electron/native-bridge/services/demoService.ts:88-349](file://electron/native-bridge/services/demoService.ts#L88-L349)
...
- 服务层（SystemService/ProjectService/CursorService/DemoService）：面向领域抽象的服务对象，负责编排适配器、维护运行时状态并暴露稳定接口。
...
- DemoBuilder 专用服务（DemoService）：专门处理演示项目的创建、管理、导入和导出操作。
...
### 服务层：System、Project、Cursor、Demo
...
- **DemoService**：专门处理演示项目的生命周期管理，包括项目创建、列表展示、加载、保存、删除、截图导入导出、编辑器窗口管理和导出功能。
...
+importScreenshot(projectId, filePath)
+pickAndImportScreenshots(projectId)
+deleteScreenshot(projectId, screenshotId, fileName)
+openDemoEditor(projectId?)
...
DemoService --> NativeBridgeStateStore : "更新Demo状态"
```
...
#### 项目管理功能
- **项目创建**：生成唯一项目ID，创建项目目录和资产目录，初始化项目配置文件
- **项目列表**：扫描用户数据目录，读取所有项目配置，按更新时间排序
- **项目加载**：从JSON文件读取项目数据，返回完整项目结构
- **项目保存**：更新项目修改时间，写入JSON文件
- **项目删除**：递归删除项目目录

#### 截图管理功能
- **截图导入**：支持PNG、JPG、JPEG、WEBP、BMP、GIF格式，自动检测图像尺寸
- **批量导入**：通过系统文件对话框选择多个文件
- **截图删除**：根据文件名查找并删除对应文件

#### 导出功能
- **视频导出**：通过Remotion框架生成视频（需要安装相关依赖）
- **GIF导出**：占位功能，待实现
- **PDF导出**：占位功能，待实现

```mermaid
flowchart TD
Start(["DemoService 调用"]) --> Create["createProject"]
Start --> List["listProjects"]
Start --> Load["loadProject"]
Start --> Save["saveProject"]
Start --> Delete["deleteProject"]
Start --> Import["importScreenshot"]
Start --> Pick["pickAndImportScreenshots"]
Start --> DeleteImg["deleteScreenshot"]
Start --> Open["openDemoEditor"]
Start --> Export["exportProject"]
...
Pick --> PickOK["批量导入完成"]
DeleteImg --> DeleteImgOK["删除完成"]
Open --> OpenOK["编辑器打开"]
Export --> ExportOK["导出完成"]
```

**图表来源**
- [electron/native-bridge/services/demoService.ts:91-126](file://electron/native-bridge/services/demoService.ts#L91-L126)
- [electron/native-bridge/services/demoService.ts:128-176](file://electron/native-bridge/services/demoService.ts#L128-L176)
- [electron/native-bridge/services/demoService.ts:213-254](file://electron/native-bridge/services/demoService.ts#L213-L254)
- [electron/native-bridge/services/demoService.ts:256-278](file://electron/native-bridge/services/demoService.ts#L256-L278)
- [electron/native-bridge/services/demoService.ts:280-300](file://electron/native-bridge/services/demoService.ts#L280-L300)
- [electron/native-bridge/services/demoService.ts:302-304](file://electron/native-bridge/services/demoService.ts#L302-L304)
- [electron/native-bridge/services/demoService.ts:306-347](file://electron/native-bridge/services/demoService.ts#L306-L347)
...
**图表来源**
- [electron/native-bridge/cursor/adapter.ts:15-20](file://electron/native-bridge/cursor/adapter.ts#L15-L20)
- [electron/native-bridge/cursor/telemetryCursorAdapter.ts:10-49](file://electron/native-bridge/cursor/telemetryCursorAdapter.ts#L10-L49)
...
**图表来源**
- [electron/native-bridge/cursor/recording/factory.ts:16-46](file://electron/native-bridge/cursor/recording/factory.ts#L16-L46)
- [electron/native-bridge/cursor/recording/session.ts:3-6](file://electron/native-bridge/cursor/recording/session.ts#L3-L6)
...
- DemoService 依赖 Electron 的文件系统和对话框 API，提供完整的演示项目管理能力。
...
IPC --> DEMO["DemoService"]
CUR --> ADP["TelemetryCursorAdapter"]
DEMO --> FS["Node.js 文件系统"]
DEMO --> DIALOG["Electron 对话框"]
```
...
- **DemoService 性能优化**：截图导入使用文件头解析快速获取图像尺寸，避免完整解码；批量导入支持异步处理多个文件。
...
- [electron/native-bridge/services/demoService.ts:40-79](file://electron/native-bridge/services/demoService.ts#L40-L79)
...
- **DemoService 故障排查**：检查用户数据目录权限，确认项目文件格式正确，验证截图格式支持列表。
...
## 结论
OpenScreen 的原生桥接系统通过服务层与适配器层的清晰分层、统一的 IPC 通道与版本化契约，实现了跨进程能力的稳定暴露与渲染器的统一消费。NativeBridgeStateStore 作为单源真相，确保了系统能力、项目上下文与游标遥测元数据的一致性与可观测性。新增的 DemoService 进一步扩展了系统能力，为 DemoBuilder 功能提供了完整的项目管理、截图处理和导出能力。该架构既满足当前需求，也为未来扩展新的原生能力提供了清晰的演进路径。
...
- **DemoService 扩展指南**：遵循 DemoService 的设计模式，使用 Electron 的文件系统 API 和对话框 API，确保项目数据的完整性和安全性。
